...
export const COMMAND_GROUPS: CommandGroup[] = [
  {
    ...
    commands: [
      {
        usage: "rooms [list|view|slots] [<room>] [flags]",
        summary: "Bookable resources — what exists, its rules and rates, and when it's free",
        flags: [
          "--type <name>      filter by resource type (list)",
          "--available        only resources available now (list)",
          "--date <when>      day to check — YYYY-MM-DD, today, tomorrow, +Nd (slots; default today)",
          "--days <n>         days of slots to fetch (slots; default 1)",
          "--interval <min>   slot granularity in minutes (slots; default 30)",
        ],
        examples: [
          "nexudus-axi rooms",
          "nexudus-axi rooms --type 'phone'",
